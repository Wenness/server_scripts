...
# sqlite3 Error voor database error afhandeling
...
def read_config(config): # Voor het aanroepen config bestand
    ...
    file_location = str(location) + '\config_insert.xml'
...
def create_connection(): # Aanmaken van de database connectie
    ...
    # Vind de database locatie in het config bestand
    db_file = read_config('databaselocatie')
    ...
    try:
        connectie = sqlite3.connect(db_file)
        return connectie
    except Error as e:
        print(e)
...
def fetch_tables(): # Voor het checken van alle tabellen in de database
    ...
    # Cursor voor SQL
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table';") # Het SQL commando gaat alle tabellen na
...
def create_new_server_table(): # Voor het aanmaken van de daadwerkelijke tabel
    ...
    sql_create_server_table = '''CREATE TABLE IF NOT EXISTS ''' + str(servernaam) + ''' (
                                    "date_today" TEXT,
	                                "time" TEXT NOT NULL,
...
if read_config('servernaam') not in fetch_tables():
